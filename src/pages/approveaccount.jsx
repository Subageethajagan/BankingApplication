...
export const sendEmail = async (toEmail, templateId) => {
  try {
    const templateParams = {
      to_email: toEmail,
      firestore_api_link: 'https://firestore.googleapis.com/v1/projects/bankingapp-41d2b/databases/(default)/documents/customerAccounts',
      // Add other template parameters here as needed
    };
 
    const serviceId = 'service_5ko2lr6'; // Replace with your EmailJS service ID
    const userId = 'E0h55fqeH_CrGR8eA'; // Replace with your EmailJS user ID
...
function ApproveAccountsPage() {
  ...
  const fetchPendingRequests = async () => {
    try {
      const response = await axios.get('https://firestore.googleapis.com/v1/projects/bankingapp-41d2b/databases/(default)/documents/customerAccounts');
  ...
  const updateRequestStatus = async (requestId, newStatus, email, phoneNumber) => {
    try {
      ...
      await axios.patch(`https://firestore.googleapis.com/v1/projects/bankingapp-41d2b/databases/(default)/documents/customerAccounts/${requestId}`, {
        fields: updatedFields
      });
 
      // Send email regardless of status
      const templateId = newStatus === 'approved' ? 'template_91ce6lw' : 'template_m7h03ce'; // Use different template for rejection
      ...
      if (newStatus === 'approved') {
        ...
        await axios.post('https://firestore.googleapis.com/v1/projects/bankingapp-41d2b/databases/(default)/documents/Account', {
          fields: {
            accountNumber: { integerValue: accountNumber },
  ...
  return (
    <div style={{ fontFamily: 'Arial, sans-serif', maxWidth: '800px', margin: '0 auto', padding: '20px' }}>
      <h1 style={{ color: '#4caf50', textAlign: 'center', marginBottom: '20px' }}>Approve Accounts</h1>
