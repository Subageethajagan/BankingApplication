...
function Transactions() {
  ...
  const fetchTransactions = async () => {
    try {
      ...
      // Filter transactions where accountNumber, senderAccountNumber, or receiverAccountNumber matches the logged-in account number
      const filteredTransactions = transactionData.filter(transaction => {
        const loggedInAccountNumberInt = parseInt(loggedInAccountNumber);
 
        return (
          (transaction.accountNumber && parseInt(transaction.accountNumber.integerValue) === loggedInAccountNumberInt) ||
          (transaction.senderAccountNumber && parseInt(transaction.senderAccountNumber.integerValue) === loggedInAccountNumberInt) ||
          (transaction.receiverAccountNumber && parseInt(transaction.receiverAccountNumber.integerValue) === loggedInAccountNumberInt) ||
          (transaction.type && transaction.type.stringValue === "Credit")
        );
      });
